...
class Barang extends MY_Controller 
{
	...
	public function barang_json()
	{
		...
		{ 
			...
			$nestedData[]	= $row['jenis_kiriman'];
			$nestedData[]	= $row['ekspedisi_awal'];

			$nestedData[]	= $row['resi_supplier'];
		...
		if($level == 'admin' OR $level == 'kasir')
		{
			if($_POST)
			{
				...
				foreach($_POST['kode'] as $kode)
				{
					...
					$this->form_validation->set_rules('id_jenis_kiriman['.$no.']','jenis_kiriman #'.($no + 1),'trim|required');
					$this->form_validation->set_rules('id_ekspedisi_awal['.$no.']','ekspedisi_awal #'.($no + 1),'trim|required');

					$this->form_validation->set_rules('resi_supplier['.$no.']','resi_supplier Barang #'.($no + 1),'trim|required|max_length[60]|alpha_numeric_spaces');
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					foreach($_POST['kode'] as $k)
					{
						...
						$id_jenis_kiriman	= $_POST['id_jenis_kiriman'][$no_array];
						$id_ekspedisi_awal	= $_POST['id_ekspedisi_awal'][$no_array];

						$resi_supplier 				= $_POST['resi_supplier'][$no_array];
						...
						$insert = $this->m_barang->tambah_baru($kode, $id_jenis_kiriman, $id_ekspedisi_awal, $resi_supplier,
							 $id_kategori_barang, $id_merk_barang, $stok, $harga, $keterangan);
						if($insert){
							$inserted++;
						}
						$no_array++;
			...
			{
				$this->load->model('m_jenis_kiriman');
				$this->load->model('m_ekspedisi_awal');

				$this->load->model('m_kategori_barang');
				$this->load->model('m_merk_barang');

				$dt['ekspedisi_awal'] = $this->m_ekspedisi_awal->get_all();
				$dt['jenis_kiriman'] = $this->m_jenis_kiriman->get_all();
			...
			if($level == 'admin' OR $level == 'kasir')
			{
				if($this->input->is_ajax_request())
				{
					...
					if($_POST)
					{
						...
						$this->form_validation->set_rules('resi_supplier','resi_supplier','trim|required|max_length[60]|alpha_numeric_spaces');
						...
						if($this->form_validation->run() == TRUE)
						{
							$resi_supplier 				= $this->input->post('resi_supplier');
							...
							$update = $this->m_barang->update_barang($id_barang, $kode_barang, $resi_supplier,  $id_kategori_barang, $id_merk_barang, $stok, $harga, $keterangan);
							if($update)
							{
								echo json_encode(array(
									'status' => 1,
	...
	public function cek_stok()
	{
		if($this->input->is_ajax_request())
		{
			...
			{
				echo json_encode(array('status' => 0, 'pesan' => "Stok untuk <b>".$get_stok->row()->resi_supplier."</b> saat ini hanya tersisa <b>".$get_stok->row()->total_stok."</b> !"));
			}
			else
			{
